...
//      `fix(ci)` or `feat(ci)` commit lands under a published section, which is
//      how v1.3.0 put "**ci:** add Tag Manager API staging push script" and two
//      Copilot-review follow-ups on the public listing.
...
//      line that restates the branch commits in vaguer words (v1.3.0 listed the
//      staging-push line twice for that reason). Dropping the merge keeps the
