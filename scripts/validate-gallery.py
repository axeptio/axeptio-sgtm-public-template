...
def check_template_info() -> None:
    ...
    # Google documents `categories` as required, but ENG-13165 established
    # empirically that it is not enforced: Usercentrics ships no `categories` and
    # its listing indexes and updates fine. So a missing or malformed value is a
    # discoverability regression, not a delisting risk, and warns rather than
    # fails. (The sibling web template's copy of this script fails instead; the
    # divergence is deliberate, not drift.) An unknown *value* still fails —
    # that is a typo, and Google has a closed list.
    categories = info.get("categories")
    if categories is None:
        warn(
            "template-categories",
            "___INFO___ has no `categories`. The gallery filters on them; pick 1-3 "
            f"from: {', '.join(sorted(ALLOWED_CATEGORIES))}",
        )
        return
    if not isinstance(categories, list) or not 1 <= len(categories) <= 3:
        warn(
            "template-categories",
            f"`categories` should be a list of 1-3 values, got {categories!r}",
        )
        return
